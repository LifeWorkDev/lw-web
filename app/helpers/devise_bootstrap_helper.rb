module DeviseBootstrapHelper
  def devise_bootstrap_error_messages!
    ...
    if flash[:alert].present?
      ...
      sentence = 'This is a demo server. All linked bank accounts & payments are fake, but emails are actually sent, so please make sure to <strong>only use email addresses that are yours.</strong>' if sentence == t('devise.failure.unauthenticated') && Rails.env.production?
    elsif messages.any?
