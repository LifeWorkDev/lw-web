class Fields::CollectionSelectWithLink < Trestle::Form::Fields::CollectionSelect
  ...
  def initialize(builder, template, name, collection, value_method, text_method, options = {}, html_options = {})
    ...
    @choices = options_for_select(collection.map { |item| [item.send(text_method), item.send(value_method), { 'data-link' => admin_link_to('View', item, class: 'float-right', target: '_blank') }] }, builder.object&.send(name))
